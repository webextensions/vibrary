...
updates works the same way, just pointed at the `-flat` branch. The mirrors are generated by
`node --run branching:flatten` (or the `/cmd-generate-flat-branches` command) and verified on demand
