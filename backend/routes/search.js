...
import { searchRunbooks } from '../utils/searchRunbooks.js';
import { sendErrorResponse, sendSuccessResponse } from '../utils/sendResponse.js';

const createSearchRouter = function ({ cwd }) {
    const router = Router();

    // Full-text search across the included runbooks files (the same set the Explorer lists). An empty query returns an
    // empty result set rather than an error, so the panel can clear its results without special-casing.
    router.get('/search', async function (request, response) {
        const query = typeof request.query.q === 'string' ? request.query.q : '';
        try {
            return sendSuccessResponse(response, await searchRunbooks(cwd, query));
        } catch {
