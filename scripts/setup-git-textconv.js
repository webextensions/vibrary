// One-time, per-clone bootstrap for the vibrary-XML diff driver. git will not let a repo commit the command a diff
...
// `.gitattributes`' `diff=vibrary-canon` resolves to `node scripts/vibrary-diff.js`.
//
...
const main = function () {
    ...
    console.log(`Wired git diff for vibrary XML files via include.path ${includeValue}.`);
};
