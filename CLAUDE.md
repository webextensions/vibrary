...
- The tarball ships only `backend`, `bin`, `dist`, the top-level `docs/*.md` manual pages, and the allowlisted
  `shared/` modules (`vibraryXmlCore.js`, `apiLimits.js`, `parseSearchQuery.js`) - the backend imports the shared
  modules at RUNTIME. A new runtime import outside those paths, or a backend-reachable package left in
